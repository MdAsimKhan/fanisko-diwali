...
function startFirecrackerAnimation(rendererCanvas: HTMLCanvasElement) {
  const c = rendererCanvas;
  if (c) {
    const ctx = c.getContext("2d");

    let cwidth: any, cheight: any;
    const shells: any = [];
    const pass: any = [];
...
placeButton.addEventListener("click", () => {
  ...
  // Start the firecracker animation
  startFirecrackerAnimation(canvas);
});
